...
import { COMMANDS, promptAttack, promptDrinkPortion, promptRun, resetGameVariables, } from "./commands.js";
    ...
    const answer = await inquirer.prompt([
        ...
        },
        {
            ...
            choices: [...opponents.keys()].map((o) => ({
                name: `${o}\t${chalk.grey(`${chalk.yellow(opponents.get(o).currHealth)} HP`)}`,
                value: o,
            })),
        },
    ]);
...
export default async function startGame(player, currOpponent) {
    ...
    if (player.currHealth <= 0) {
        ...
        console.clear();
        console.log(chalk.whiteBright.bold(`\n\n\n\t\t-: The Winner is ${chalk.greenBright.bold.italic(player.name)} :-`));
        await endGame();
    }
    else {
